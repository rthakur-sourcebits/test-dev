...
class Dropbox_API {
	...
	/**
     * This method is used search and find exactly where the device name is stored in Dropbox
     * 
     * @param string $device
     * @param array $arguments 
     * @return bool 
     */
	public function search($device = '', $root = null, $path = '') {
		$arr_path	=	array();
		$device = rawurldecode($device);
		if (is_null($root)) $root = $this->root;
        $response = $this->oauth->fetch('http://api.dropbox.com/0/search/' . $root . '/' ,array('query' => $device));
		
		if(empty($response['body'])) return false;
		$body			=	json_decode($response['body'], true);
        $total_result	=	count($body);
        for($i=0;$i<$total_result;$i++) {
        	$arr_path[]		=	$body[$i]['path'];
        }
       
        if(in_array("/AccountEdge/".$device, $arr_path)) { //echo "Device Name - ".$device;echo "<pre>"; print_r($arr_path);
        	return false;
        } else if(in_array("/Apps/AccountEdge/".$device, $arr_path)){//echo "Device Name - ".$device;echo "<pre>"; print_r($arr_path);
        	return true;
        } else {
        	return false;
        }
