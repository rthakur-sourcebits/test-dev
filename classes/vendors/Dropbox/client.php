...
class DropboxClient
{
     ...
     * @return array
     */
    public function getFile($path, $outFile = "", $revision = null) {
		...
		//echo "<pre>";print_r($response);die;
		//$file_content	=	json_decode(utf8_encode($response['body']));
	...
	/**
     * Creates a folder to the user's Dropbox
     *
     * The path is relative to a root (ex /<root>/<path>) that can be 'sandbox' or 'dropbox'
     *
     * @param  string   $file       The full path of the file to upload
     ...
     * @return array
     */
    public function create_folder($path = "/", $name = null, $overwrite = true) {
           // Get the raw response body
$path = str_replace('%2F','/',rawurlencode($path));
        $response = $this->Session->fetch("POST", $this->dropboxAPIURL, "/fileops/create_folder",array('root'=>'dropbox','path'=>$path));
        return json_decode($response['body'],true);
    }
    ...
    public function search($query, $path = '/', $limit = 1000, $deleted = false){
	 ...
	 $path1 = str_replace('%2F','/',rawurlencode($path1));
        $response = $this->Session->fetch('GET', $this->dropboxAPIURL, "/search/".$path1, $params);
	
	if(count($response['body'])==0) {return false;}
		
	 $body			=	json_decode($response['body'], true);
        $total_result	=	count($body);
        
        if(count($total_result)>0){
        	if(isset($body[0]['path'])){
	        for($i=0;$i<$total_result;$i++) {
	        	$arr_path[]		=	strtolower($body[$i]['path']);
	        }
			
	        if(in_array(strtolower($path.$query), $arr_path)) { 
	           	return true;
	        } else {
	        	return false;
	        }
...
/**
     ...
     */
	 public function delete($path="/"){
	      // Get the raw response body
	      //$path = str_replace('%2F','/',rawurlencode($path));
	      $response = $this->Session->fetch("POST", $this->dropboxAPIURL, "/fileops/delete",array('root'=>'dropbox','path'=>$path));
	      return json_decode($response['body'],true);
	 }
