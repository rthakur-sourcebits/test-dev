 ...
 * @Class : Controller_Aecapi
 * @Description: This class file holds the operations of aec api.
...
class Controller_Aecapi extends Controller {
	
 	/**
	 * @Method: __construct 
	 * @Description: This method calls the validation session and checks user is authorized to access the AEC API from OAuth
	...
	public function action_testLogin(){
		?>
		<form enctype='multipart/form-data' method='POST' action='<?php echo SITEURL; ?>/aecapi/loginUser' >
			<table>
			<tr>
	...
	public function action_sync(){  
		...
		// in  practice we need to test and modify this based on the exactly implementation of POST FILE access. 
		if(isset($_POST['json'])){
			$contents = $_POST['json'];
			...
			if(isset($_GET['aec_token'])&&isset($_GET['aec_secret'])&& $oauth_server_lib->sign_request($_GET['aec_token'], $_GET['aec_secret'])){ // for now we are using session otherwise will go for GET request variable.			
				...
				if(isset($flag_data)&& isset($flag_data['flag']) && !$flag_data['flag']){			
					$oauth_server_lib->access_user_flag($_GET['aec_token'], $_GET['aec_secret'], 1); // set the user access flag : LOCK
					...
					if(isset($module)&&isset($company_id)){					
						switch($module){
							case "customer":
								...
								if(isset($contents)&&!empty($content)){
							...
							default:
									...
									$response["error"] 		= Kohana::message('error', 'aecapi4'); // no module
								break;
						}					
					}else{
						$response["success"]	= FALSE;
					...
					$response["error"] 		= Kohana::message('error', 'aecapi5'); // access error
				}			
			} else {			
				$response["success"]	= FALSE;
				$response["error_code"]	= 6;
				$response["error"] 		= Kohana::message('error', 'aecapi6'); // not authorized user
			}
		}else{
			$response["success"]	= FALSE;
			$response["error_code"]	= 2;
			$response["error"] 		= Kohana::message('error', 'aecapi2'); // no json file
		}
			...
			if(isset($_POST)){
				...
				if(!$init_force){
					$aec_updates		= $aecapi_customer_m->update_customers($company_id, $content); // here I have to check the conflicted records as well and marked it as un-synced.
																	// if the data is marked as unsync then, will not update the data and will leave it as it is.
					...
					if ($init_force == AED_FORCE_SYNC){
						$res['master']['init_force_sync']=false;
						...
						// this will make AED to send us all the data.
	...
	{
		echo " 	<form enctype='multipart/form-data' method='POST' action='".SITEURL."/aecapi/sync?module=".$_GET['module']."&aec_token=".$_GET['aec_token']."&aec_secret=".$_GET['aec_secret']."' >
